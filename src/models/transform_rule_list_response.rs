/*
 * HookSniff API
 *
 * Webhook delivery, monitoring, and management API. All endpoints under `/v1` require authentication via `Authorization: Bearer <api_key>` header unless marked as **Public**. 
 *
 * The version of the OpenAPI document: 1.0.0
 * Contact: support@hooksniff.vercel.app
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

/// TransformRuleListResponse : Paginated list of transform rules
#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct TransformRuleListResponse {
    #[serde(rename = "data")]
    pub data: Vec<models::TransformRule>,
}

impl TransformRuleListResponse {
    /// Paginated list of transform rules
    pub fn new(data: Vec<models::TransformRule>) -> TransformRuleListResponse {
        TransformRuleListResponse {
            data,
        }
    }
}

