...
impl Default for HookSniffOptions {
    fn default() -> Self {
        Self {
            debug: false,
            server_url: None,
...
impl HookSniff {
    pub fn new(token: String, options: Option<HookSniffOptions>) -> Self {
        let options = options.unwrap_or_default();

        let cfg = Arc::new(Configuration {
            user_agent: Some(format!("hooksniff-libs/{CRATE_VERSION}/rust")),
        ...
        hooksniff.with_token(token)
    }

    /// Creates a new `HookSniff` API client with a different token,
    /// re-using all of the settings and the Hyper client from
    /// an existing `HookSniff` instance.
    ///
