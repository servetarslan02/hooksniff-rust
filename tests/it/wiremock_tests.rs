use hooksniff::api::{MessageListOptions, HookSniff, HookSniffOptions};
...
async fn test_idempotency_key_is_sent_for_create_request() {
    ...
    svx.application()
        .create(hooksniff::api::ApplicationIn::new("test app".to_string()), None)
        .await
        .unwrap();
...
async fn test_client_provided_idempotency_key_is_not_overridden() {
    ...
    svx.application()
        .create(
            hooksniff::api::ApplicationIn::new("test app".to_string()),
            Some(hooksniff::api::ApplicationCreateOptions {
